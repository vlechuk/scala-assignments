...
object Huffman {
    ...
    def times(chars: List[Char]): List[(Char, Int)] = {
      ...
      def count_chars(char: Char, chars_occ_list: List[(Char, Int)]): List[(Char, Int)] = {
        if(chars_occ_list.isEmpty)
         (char,1) :: chars_occ_list
         // chars_occ_list :+ (char, 1)
        else {
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      ...
      def makeLeafs(l: List[(Char, Int)], ll: List[Leaf]): List[Leaf] = {
        if(!l.isEmpty)
          makeLeafs(l.tail, Leaf(l.head._1,l.head._2) :: ll)
        else ll
      }
      ...
      if(freqs.isEmpty)
        ...
        makeLeafs(freqs.sortWith(_._2>_._2), List[Leaf]())
    }
   ...
   */
    def until(f_singletone: (List[CodeTree]) => Boolean, f_comb: (List[CodeTree]) => List[CodeTree])(trees: List[CodeTree]): CodeTree = {
      if(f_singletone(trees))
        trees.head
      else
        until(f_singletone, f_comb)(f_comb(trees))
    }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def help(sub_tree: CodeTree, bits: List[Bit]): List[Char] = {
      sub_tree match {
        case Leaf(char, _) => char :: help(tree, bits)
        case Fork(left, right, _, _) => {
          if( bits.isEmpty ) List[Char]()
          else bits.head match {
            case 0 => help(left, bits.tail)
            case 1 => help(right, bits.tail)
          }
        }
      }
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def help(sub_tree: CodeTree, text: List[Char]): List[Bit] = {
      if( text.isEmpty ) List[Bit]()
      else sub_tree match {
        case Leaf(_, _) => help(tree, text.tail)
        case Fork(left, right, _, _) =>
   ...
   */
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      val retrieve_bits = codeBits(convert(tree))_
      def quickEncodeHelp(text: List[Char]): List[Bit] = {
        if( text.isEmpty ) List[Bit]()
        else retrieve_bits(text.head) ::: quickEncodeHelp(text.tail)
      }
